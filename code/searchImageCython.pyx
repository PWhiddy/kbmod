...
class searchImageCython(object):
    ...
    def createMask(self, image_folder, threshold):
        ...
        self.im_shape = np.shape(mask)

        return mask_image

    def calcPsiPhi(self, char *image_folder, np.ndarray mask_array):
        ...
        cdef int num_images = len(os.listdir(image_folder))
        cdef np.ndarray psi_array = np.zeros([num_images, self.im_shape[0], self.im_shape[1]], dtype=DTYPE)
        cdef np.ndarray phi_array = np.zeros([num_images, self.im_shape[0], self.im_shape[1]], dtype=DTYPE)
        cdef list filename
        cdef int idx
        ...
        filename = sorted(os.listdir(image_folder))

        for idx in range(num_images):

            print str('On Image ' + filename[idx])

            image_file = os.path.join(image_folder, filename[idx])
            exposure = afwImage.ExposureF(image_file)
